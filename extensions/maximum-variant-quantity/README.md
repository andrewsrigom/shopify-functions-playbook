...
Limit each product variant across all cart lines.

Quantities of the same variant are summed even when line attributes split it into separate lines. Different variants have separate limits. Exactly the maximum is allowed.
...
25 validations per store; test alongside any other quantity restrictions.
...
pnpm typegen maximum-variant-quantity
pnpm typecheck
pnpm test -- extensions/maximum-variant-quantity/tests
pnpm build maximum-variant-quantity
pnpm test:runtime maximum-variant-quantity
```
