...
Reward existing Y units for each complete group of X units.

buyQuantity X units qualify getQuantity Y units at 100% off. Qualifying and rewarded variant IDs must differ. Rewards are capped by Y units already present and allocated in cart-line order. The Function cannot add a gift to the cart. For two X and one Y configured, four X qualify two Y.
...
25 discount functions per store; these examples set all combinesWith fields to false.
...
pnpm typegen buy-x-get-y
pnpm typecheck
pnpm test -- extensions/buy-x-get-y/tests
pnpm build buy-x-get-y
pnpm test:runtime buy-x-get-y
```
