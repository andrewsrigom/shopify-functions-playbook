...
Discount a named delivery option when the cart reaches a currency-specific subtotal.

Matches an exact option title in each delivery group, then targets its runtime handle. At the threshold, the configured integer percentage applies. Missing titles/groups produce no candidate. SHIPPING class is required.

## Prerequisites and restrictions

API **2026-04**, target `cart.delivery-options.discounts.generate.run`. Local: Node.js 24 and pnpm 10.32.0. Live: app development permissions, an app linked with Shopify CLI, and a suitable development store. Public apps containing Functions can serve supported plans; custom apps containing Function APIs require Shopify Plus on production stores. Development-store capabilities must be checked before activation.

25 discount functions per store. Carrier/localized option titles can change; adapt matching to store-specific semantics.

## Read the code

Read `config.example.json`, `src/cart_delivery_options_discounts_generate_run.graphql`, `generated/input.ts`, `src/cart_delivery_options_discounts_generate_run.ts`, then `fixtures/` and `tests/run.test.ts`. The implementation is the main entry point; `src/index.ts` exports it to the compiler. `generated/api.ts` contains schema output types. Input queries select Shopify-provided runtime data; they are not arbitrary Admin API requests.
...
pnpm typegen conditional-shipping-discount
pnpm typecheck
pnpm test -- extensions/conditional-shipping-discount/tests
pnpm build conditional-shipping-discount
pnpm test:runtime conditional-shipping-discount
```
