...
Prevent two configured products from being bought together.

Matches product GIDs across all variants. Either product alone is allowed. Identical configured product IDs are invalid.
...
25 validations per store; all active validation errors can apply.
...
pnpm typegen incompatible-products
pnpm typecheck
pnpm test -- extensions/incompatible-products/tests
pnpm build incompatible-products
pnpm test:runtime incompatible-products
```
