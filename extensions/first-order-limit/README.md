...
Limit subtotal for buyers without a completed order history.

Uses customer.numberOfOrders supplied by Shopify. Known customers with zero orders are limited. Guest/unidentified buyers follow explicit guests=limit or guests=allow. This is not identity verification; guest checkout cannot reliably establish a person’s purchase history. Exact maximum is allowed.
...
25 validations per store; access to customer input and actual checkout behavior must be verified in the development store.
...
pnpm typegen first-order-limit
pnpm typecheck
pnpm test -- extensions/first-order-limit/tests
pnpm build first-order-limit
pnpm test:runtime first-order-limit
```
