...
Discount existing cart lines for a customer already tagged as VIP in Shopify.

The query fixes the tag playbook-vip. It is a fixture classification already stored in Shopify, not an implemented external integration. Guests, absent customers, and untagged customers get no discount. PRODUCT class is required.
...
25 discount functions per store; disable other discount examples or explicitly review combinations.
...
pnpm typegen vip-discount
pnpm typecheck
pnpm test -- extensions/vip-discount/tests
pnpm build vip-discount
pnpm test:runtime vip-discount
```
